//Creating on 2016/07/18 by simona  for cashRedPack
'use strict';
...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		useminPrepare: {
			...
			html: ['<%= config.activity %>/<%= config.actName %>/html/index.html', '<%= config.activity %>/<%= config.actName %>/html/distribute.html']
		},
		...
		cssmin: {
			dist: {
				files: {
					...
					'<%= config.activity %>/<%= config.actName %>/<%= config.dist %>/css/distribute.css': [
						'<%= config.activity %>/<%= config.actName %>/.tmp/css/distribute.css',
					]
				}
			}
		},
